...
mod tests {
    ...
    #[test]
    fn valid_keeps_legacy_filtering_rules() {
        assert!(!valid(""));
        assert!(!valid("! comment"));
        assert!(!valid("# comment"));
        assert!(!valid("[Adblock Plus 2.0]"));
        assert!(!valid("$removeparam=utm_source"));
        assert!(!valid("/banner.js"));
        assert!(valid("||example.com^"));
        assert!(valid("@@||example.com^"));
    }

    #[test]
    fn domain_key_matches_adblock_and_hosts_rules() {
        assert_eq!(
            domain_key("||ads.example.com^").as_deref(),
            Some("ads.example.com")
        );
        assert_eq!(
            domain_key("@@||allowed.example.com^").as_deref(),
            Some("allowed.example.com")
        );
        assert_eq!(
            domain_key("127.0.0.1 tracker.example.net").as_deref(),
            Some("tracker.example.net")
        );
        assert_eq!(
            domain_key("plain.example.org").as_deref(),
            Some("plain.example.org")
        );
        assert_eq!(domain_key("example.com##.ad"), None);
    }
}
